...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      description:
        'Checks the guild for any user accounts younger than the minimum account age.'
    });
  }
  ...
  async run(msg) {
    ...
    for (const member of msg.guild.members.values()) {
      ...
      users.push(
        `${member.user.tag}, Created:${(
          (mtime - member.user.createdTimestamp) /
          1000 /
          60
        ).toFixed(0)} min(s) ago`
      );
    ...
    return msg.sendMessage(
      users.length > 0
        ? `The following users are less than the Minimum Account Age:${util.codeBlock(
            '',
            users.join('\n')
          )}`
